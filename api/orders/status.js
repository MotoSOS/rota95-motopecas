...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabaseRest(`/orders?external_id=eq.${encodeURIComponent(order)}&select=external_id,status,fulfillment_status,payment_id,payment_method,total,invoice_url,payment_data,tracking_code,carrier,tracking_url,admin_notes,paid_at,shipped_at,delivered_at,updated_at&limit=1`);
