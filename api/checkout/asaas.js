...
import {
  ...
  publicOrigin, clientIp, authenticatedUser, supabaseRest, storeValue, asaas, addOrderHistory
} from '../_lib.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const existing = await supabaseRest(`/orders?external_id=eq.${encodeURIComponent(externalId)}&select=id,external_id,payment_id,status,total,invoice_url,payment_data,fulfillment_status&limit=1`);
    if (Array.isArray(existing) && existing[0]?.payment_id) {
      return sendJson(res, 200, {
        orderId: existing[0].external_id,
