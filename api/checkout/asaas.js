...
import {
  ...
  clientIp, authenticatedUser, supabaseRest, storeValue, asaas, addOrderHistory, assignDisplayId
} from '../_lib.js';
...
function basePaymentBody({ customerId, externalId, method, installments, total }) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const existing = await supabaseRest(`/orders?external_id=eq.${encodeURIComponent(externalId)}&select=id,external_id,display_id,payment_id,status,total,invoice_url,payment_data,payment_method,fulfillment_status&limit=1`);
    if (Array.isArray(existing) && existing[0]?.payment_id) {
      let paymentUi = existing[0].payment_data || null;
      let invoiceUrl = existing[0].invoice_url || '';
      let status = existing[0].status || 'PENDING';
      if (!paymentUi) {
        try {
          const charge = await asaas(`/payments/${encodeURIComponent(existing[0].payment_id)}`);
          status = charge.status || status;
          invoiceUrl = charge.invoiceUrl || charge.bankSlipUrl || invoiceUrl;
          paymentUi = await paymentPresentation(existing[0].payment_method || payment.method, charge);
          await supabaseRest(`/orders?id=eq.${encodeURIComponent(existing[0].id)}`, {
            method: 'PATCH', prefer: 'return=minimal',
            body: { status, invoice_url: invoiceUrl, payment_data: paymentUi, updated_at: new Date().toISOString() }
          });
        } catch (presentationError) {
          console.warn('[ROTA95 checkout] Cobrança existente sem apresentação:', presentationError?.message || presentationError);
        }
      }
    ...
    // Salva a cobrança antes de buscar QR Code/linha digitável. Assim, se o Asaas
    // tiver uma falha transitória na apresentação, o pedido não é duplicado na nova tentativa.
    ...
    try {
      paymentUi = await paymentPresentation(payment.method, charge);
      await supabaseRest(`/orders?id=eq.${encodeURIComponent(orderDbId)}`, {
        method: 'PATCH', prefer: 'return=minimal',
        body: { payment_data: paymentUi, updated_at: new Date().toISOString() }
      });
    } catch (presentationError) {
      console.error('[ROTA95 checkout presentation]', presentationError?.message || presentationError);
      presentationWarning = payment.method === 'pix'
        ? 'A cobrança Pix foi criada, mas o Asaas não liberou o QR Code. Verifique se existe uma chave Pix ativa na conta Sandbox e tente novamente em alguns segundos.'
        : payment.method === 'boleto'
          ? 'O boleto foi criado, mas a linha digitável ainda não foi liberada pelo Asaas. Tente novamente em alguns segundos.'
          : 'A cobrança foi criada, mas os dados de apresentação ainda não foram liberados.';
      paymentUi = {
        type: payment.method,
        unavailable: true,
      ...
      await addOrderHistory(orderDbId, 'PAYMENT_PRESENTATION_ERROR', charge.status || 'PENDING', presentationWarning, { paymentId: charge.id }, 'Sistema');
    }
