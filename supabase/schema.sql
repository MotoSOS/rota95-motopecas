...
-- Apenas o administrador autenticado pelo Google pode alterar catálogo e configurações.
-- Troque o e-mail abaixo caso a conta administrativa seja outra.
drop policy if exists "admin write store state" on public.store_state;
create policy "admin write store state" on public.store_state
for all
using ((auth.jwt() ->> 'email') = 'motopecas.rota95@gmail.com')
