...
## Importar os produtos já cadastrados

Este pacote contém **23 produtos** e **9 categorias** recuperados do backup de 2026-07-21T05:10:47.520Z.

1. Execute primeiro `supabase/schema.sql` no SQL Editor.
2. Execute `supabase/import-backup-2026-07-21.sql`.
3. Confirme que a consulta final retorna 7 registros.
4. Na Vercel, faça um novo deploy depois de enviar todos os arquivos ao GitHub.

O arquivo `initial-state.js` também carrega os produtos no primeiro acesso, mesmo antes da importação. A base Supabase passa a ser a fonte compartilhada depois que o SQL de importação é executado.

Para salvar alterações feitas no painel administrativo em todos os aparelhos, entre com o Google usando `motopecas.rota95@gmail.com`. Essa conta está autorizada pela política RLS definida no schema.
